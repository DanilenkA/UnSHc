# Если нужно положить закрытый bash скрипт, чтобы юзвери не смогли видеть его содержимое, то можно вот так

# на машине ставим apt install shc gcc, и компилим через shc -f -r script.sh, на выходе получим - script.sh.x - бинарник
# - script.sh.x.c - исходный код. бинарь будет работать точно так же, как и просто sh. Также можно указать дату, после которой скрипт больше не запуститься, причём он выведет сообщение, которое можно указать
# shc -e 31/12/2025 -m "Этот файл протух, обратитесь к разработчику за новым" -f -r script.sh
...
Это софтина расшифровывает такие скрипты
